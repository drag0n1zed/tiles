...
use crate::grid::{AnimationState, Grid, tile::Tile};

#[derive(Serialize, Deserialize)]
pub(super) struct VecGrid {
    height: usize,
    width: usize,
...
impl From<VecGrid> for Grid {
    fn from(vec_grid: VecGrid) -> Self {
        ...
        Grid {
            array: Array2::from_shape_vec((vec_grid.height, vec_grid.width), vec_flat).unwrap(),
            active_animations: vec![],
            animation_mask: Array2::from_elem(
                (vec_grid.height, vec_grid.width),
                AnimationState::None,
            ),
        }
    }
